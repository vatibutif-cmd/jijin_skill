...
> **版本**: v2.2 | **日期**: 2026-08-04 | **数据源**: AKShare + 东方财富实时推送 + A股数据层(腾讯/东财)

---

## 一、设计原则

1. **核心 14 个工具**（基金 10 工具 + A股数据 4 工具），命名空间 `famas-data`，覆盖 10 个 Agent 全链路数据与消息推送需求
2. **每个工具只做一件事**，输入输出边界清晰，不交叉
3. **AKShare 原生接口 + 实时推送 + A股直连 一对一映射**，高效稳定
...
## 六、A股数据工具（4 个，V2.2 新增）

> 集成自开源项目 [a-stock-data](https://github.com/simonlin1212/a-stock-data)（Apache-2.0, V3.6.0）。
> 为基金分析补充**个股/市场级**数据：重仓股行情、行业资金流、行业轮动、重仓股新闻。
> 实现位于 `mcp_server/a_stock_data/`，腾讯接口不封 IP，东财接口内置 `em_get()` 限流防封。

### 工具 A1: `a_stock_quote_tool`

**功能**: 批量获取 A股/指数/ETF 实时行情（腾讯财经）。
**参数**: `codes`（逗号分隔 6 位代码，≤20 个）
**返回**: 每代码的现价、涨跌幅、PE/PB、市值、换手率、振幅、量比、僵尸报价标记
**用途**: 基金重仓股实时估值、指数行情锚定、ETF 联接标的校验

### 工具 A2: `a_stock_board_fund_flow`

**功能**: 板块资金流向排名（东方财富）。
**参数**: `board_type`(industry/concept/region)、`period`(today/5d/10d)、`top_n`
**返回**: 按主力净流入降序的板块列表，含主力净额/净占比、涨跌幅、领涨股、四档单明细(今日)
**用途**: `sector_screener` 行业筛选的资金面验证、`macro_strategist` 风格轮动判断

### 工具 A3: `a_stock_industry_rank`

**功能**: 全行业涨跌幅排名（东方财富，~100 个行业）。
**参数**: `top_n`
**返回**: 涨幅 TOP 榜 + 跌幅 BOTTOM 榜，含上涨/下跌家数、领涨股
**用途**: `macro_strategist` 市场风格定位、行业轮动强度评估

### 工具 A4: `a_stock_stock_news`

**功能**: 个股新闻（东方财富）。
**参数**: `code`、`page_size`
**返回**: 新闻列表（日期/标题/来源/链接）
**用途**: `watchtower` 持仓穿透时追踪重仓股动态、`portfolio_doctor` 组合风险监测

### Agent → A股工具映射

| Agent | A股工具 | 场景 |
|-------|---------|------|
| sector_screener | A2, A3 | 行业资金流验证、行业热度 |
| macro_strategist | A3 | 风格轮动、行业排名 |
| portfolio_doctor | A1, A4 | 重仓股估值、个股异动 |
| watchtower | A1, A4 | 重仓股监控、新闻预警 |
| fund_comparator | A1 | 持仓重合股估值对比 |
...
# 完整 A股能力（可选：mootdx 通达信 K线 + stockstats 技术指标）
pip install mootdx stockstats
```

### 沙箱网络说明

腾讯财经接口（`qt.gtimg.cn`）不封 IP、沙箱可直连；东方财富接口（`push2.eastmoney.com`）在部分受限网络可能被代理拦截，生产环境如有问题请参照 `docs/etfirst-network-fix.md` 绕过代理或调整网络策略。
